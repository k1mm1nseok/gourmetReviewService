...
| **is_phone_verified** | BOOLEAN | Y | **[리뷰 작성 권한]** 휴대폰 인증 완료 여부. `FALSE`이면 리뷰 작성 API는 거부되어야 한다. |
...
| 필드명 | 타입 | 필수여부 | 설명 및 제약조건 |
|:---|:---|:---|:---|
| **score_weighted** | DECIMAL(3,2) | Y | **[최종 평점]** 정책 v1.3.3 알고리즘에 의해 계산된 결과값.<br>조회 성능을 위해 실시간 계산이 아닌 저장된 값을 사용(캐싱). |
...
1.  **작성 직후 (`PENDING`)**
    ...
    * **초기 운영 모드:** 서비스 런칭 초기에는 모든 리뷰가 운영자 승인 대기.
2.  **검수 단계**
...
3.  **공개 단계**
    ...
    * **방문횟수 반영:** 리뷰가 `PUBLIC`으로 전환되는 시점에 해당 회원-가게의 누적 방문횟수(`member_store_visit.visit_count`)를 +1 하고,
      전환된 리뷰의 `visit_count`에 반영한다.
4.  **제재 단계**
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|-----|------|----------|-------|
| 1.0 | 2025-12-10 | 최초 작성 (리뷰 정책 v1.3.3 기반) | System |
